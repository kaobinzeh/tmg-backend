...
public sealed class WhenCompletingPasswordResetWithValidOtp_Should(ContainersFixture fixture)
    : WebApiIntegrationTestBase(fixture), IAsyncLifetime
{
    private const string OldPassword = "P@ssw0rd123!";
    private const string NewPassword = "N3wP@ssw0rd123!";
    ...
    {
        await WhenCompletingPasswordReset();
        await ThenThePasswordIsUpdated();

        async Task WhenCompletingPasswordReset()
        {
            _response = await Client.PostAsJsonAsync(
                EndpointUrl.PasswordResets.CompletionsV1,
                new CompletePasswordResetRequest(_email, _otp, NewPassword, NewPassword));
        }

        async Task ThenThePasswordIsUpdated()
        {
            ...
            (await identityService.CheckPasswordAsync(user, OldPassword)).ShouldBeFalse();
            (await identityService.CheckPasswordAsync(user, NewPassword)).ShouldBeTrue();
        }
    }

    private async Task CreateUserAndOtpAsync()
    {
