...
public sealed class WhenUsingProtectedEndpointAfterLogout_Should(ContainersFixture fixture)
    : WebApiIntegrationTestBase(fixture), IAsyncLifetime
{
    private const string Password = "P@ssw0rd123!";
    private static readonly string ProfileEndpoint = $"{EndpointUrl.Stakeholders.V1}/me/profile";
    ...
    private HttpResponseMessage? _updateProfileBeforeLogoutResponse;
    private HttpResponseMessage? _logoutResponse;
    private HttpResponseMessage? _updateProfileAfterLogoutResponse;
    ...
    public async Task DisposeAsync()
    {
        ...
        _updateProfileBeforeLogoutResponse?.Dispose();
        _logoutResponse?.Dispose();
        _updateProfileAfterLogoutResponse?.Dispose();
        await DeleteAuthenticationRecordsAsync();
        await DisposeClientAsync();
    }

    [Fact]
    public async Task RejectTheAccessToken()
    {
        SignInResponse? signInPayload = default;

        await WhenSigningIn();
        await WhenCallingProtectedEndpointBeforeLogout();
        await WhenLoggingOut();
        await WhenCallingProtectedEndpointAfterLogout();
        ThenProtectedEndpointAccessIsRevoked();
        ...
        async Task WhenCallingProtectedEndpointBeforeLogout()
        {
            _updateProfileBeforeLogoutResponse = await Client.PutAsJsonAsync(
                ProfileEndpoint,
                new UpdateProfileRequest("Updated", "Profile"));
        }
        ...
        async Task WhenCallingProtectedEndpointAfterLogout()
        {
            _updateProfileAfterLogoutResponse = await Client.PutAsJsonAsync(
                ProfileEndpoint,
                new UpdateProfileRequest("Rejected", "Request"));
        }

        void ThenProtectedEndpointAccessIsRevoked()
        {
            _signInResponse.ShouldNotBeNull();
            _updateProfileBeforeLogoutResponse.ShouldNotBeNull();
            _logoutResponse.ShouldNotBeNull();
            _updateProfileAfterLogoutResponse.ShouldNotBeNull();
